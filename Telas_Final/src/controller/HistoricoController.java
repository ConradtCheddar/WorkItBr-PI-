...
public class HistoricoController extends ComponentAdapter {
	...
	public HistoricoController(TelaHistorico view, UsuarioDAO model, Navegador navegador, ServicoDAO servicoDAO,
				...
				if (e.getClickCount() == 2) {
					...
					if (selectedItem instanceof Servico) {
						if (navegador.getCurrentUser().isContratado()) {
							...
							String panelName = telaFactory.criarVisServicoFinalizado(servicoSelecionado);
							navegador.navegarPara(panelName);
							return;
						}
						if (navegador.getCurrentUser().isContratante()) {
							Servico servicoSelecionado = (Servico) selectedItem;
							String panelName = telaFactory.criarVisServicoCnteFinalizado(servicoSelecionado);
							navegador.navegarPara(panelName);
							return;
						}
